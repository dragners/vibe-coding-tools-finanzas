...
function MetricTable<T extends string>({
  ...
  const gridTemplate = { gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` } as const;

  return (
    <div className="rounded-md border border-slate-200 overflow-hidden bg-white">
      <div className="bg-slate-50 px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-slate-500 text-center">
        {title}
      </div>
      <div className="border-t border-slate-200">
        <div
          className="grid text-[11px] font-semibold uppercase tracking-wide text-slate-500"
          style={gridTemplate}
        >
          {labels.map((label) => (
            <div
              key={`header-${label}`}
              className="px-2 py-1 text-center border-r border-slate-200 last:border-r-0"
            >
              {label}
            </div>
          ))}
        ...
        <div className="border-t border-slate-200" />
        <div className="grid text-sm text-slate-700" style={gridTemplate}>
          {labels.map((label) => (
            <div
              key={`value-${label}`}
        ...
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-50">
            <tr className="text-left text-xs font-semibold text-slate-600 uppercase tracking-wide">
              ...
              <th className="px-4 py-3 min-w-[220px]">{texts.performance}</th>
              <th className="px-4 py-3 min-w-[180px] whitespace-nowrap">{texts.sharpe}</th>
              <th className="px-4 py-3 min-w-[180px] whitespace-nowrap">{texts.volatility}</th>
            ...
            {data.length === 0 ? (
              ...
              data.map((row) => (
                <tr key={`${section}-${row.morningstarId}`} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 align-top">
                    <MetricTable
                      title={texts.performance}
                      labels={PERFORMANCE_LABELS}
                      values={row.performance}
                    />
                  </td>
                  <td className="px-4 py-3 align-top">
                    <MetricTable
                  ...
                  <td className="px-4 py-3 align-top">
                    <MetricTable
                      title={texts.volatility}
                      labels={RATIO_LABELS}
                      values={row.volatility}
                    />
                  </td>
        ...
        <div className="max-w-6xl mx-auto px-4 py-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              {status === "ready" && lastUpdatedText && (
                <p className="text-xs text-slate-500">
                  {texts.lastUpdated}: {lastUpdatedText}
                </p>
              )}
            </div>
          </div>
